...
import { SignIn, SignOut } from '@/app/components/AuthComponents'
...
export default async function AppLayout({
  ...
  if (session?.user?.id) {
    ...
    if (allTaskIds.length > 0) {
      ...
      pendingTasksCount = allTaskIds.length - todayCompletions.length
    }
  }
              ...
              {session?.user ? (
                <>
                  ...
                  <nav style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-2)' }}>
                    <Link href="/dashboard" className="btn btn-ghost" style={{ justifyContent: 'flex-start' }}>
                      Dashboard
                      {pendingTasksCount > 0 && (
                        <span style={{ 
                          marginLeft: 'auto', 
